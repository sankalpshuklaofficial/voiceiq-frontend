...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    ...
    businessName: "", businessType: "", agreeTerms: false,
  });
        ...
        <div className="relative space-y-6">
          ...
          <div className="space-y-4">
            {[
              ...
              "Hindi + English AI out of the box",
          ...
          <div className="bg-white/10 backdrop-blur rounded-2xl p-5 border border-white/20">
            ...
            <div className="flex gap-3 items-center">
              {[{ n: 1, label: "Your Details" }, { n: 2, label: "Business Info" }].map((s, i) => (
                <div key={s.n} className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                    step > s.n ? "bg-green-400 text-white" :
                    step === s.n ? "bg-white text-blue-700" :
                    "bg-white/20 text-white/50"
                  }`}>
                    {step > s.n ? "✓" : s.n}
                  </div>
                  <span className={`text-xs ${step >= s.n ? "text-white" : "text-white/50"}`}>{s.label}</span>
                  {i === 0 && <div className={`w-8 h-0.5 ${step > 1 ? "bg-green-400" : "bg-white/20"}`} />}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-400 mt-2">
              {step === 1 ? "Start your 14-day free trial today" : "Help us customize your AI receptionist"}
            </p>
          </div>
          ...
          {step === 1 && (
            <div className="space-y-4">
              {[
                { key: "name", label: "Full Name", icon: "👤", type: "text", placeholder: "Sankalp Sharma" },
                { key: "email", label: "Email Address", icon: "📧", type: "email", placeholder: "you@example.com" },
                { key: "phone", label: "Phone Number", icon: "📱", type: "tel", placeholder: "+91 98765 43210" },
              ].map(f => (
                <div key={f.key}>
                  <label className="text-sm font-medium text-gray-700 block mb-1.5">{f.label}</label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">{f.icon}</span>
                    <input
                      type={f.type}
              ...
              <button onClick={handleNext} className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-semibold text-sm hover:bg-blue-700 transition shadow-md shadow-blue-200 mt-2">
                Continue →
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div className="flex gap-3">
                <button onClick={() => { setStep(1); setError(""); }} className="flex-1 border border-gray-200 text-gray-600 py-3.5 rounded-xl text-sm font-medium hover:bg-gray-100 transition">
                  ← Back
                </button>
                <button
                  onClick={handleRegister}
                  disabled={loading}
                  className="flex-1 bg-blue-600 text-white py-3.5 rounded-xl font-semibold text-sm hover:bg-blue-700 transition shadow-md disabled:opacity-60 flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
